 ...
 * Versiune bilingvă completă (RO/EN)
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Verificăm captcha fără a-l șterge imediat din sesiune
    ...
    if ($captcha_expected === null || $captcha_answer !== (int)$captcha_expected) {
        $errors[] = isset($lang['cont_err_captcha']) ? $lang['cont_err_captcha'] : 'Verificarea a eșuat. Te rugăm să încerci din nou adunarea.';
    }
    ...
    if (empty($errors)) {
        ...
        if (@mail($contact_recipient, '=?UTF-8?B?' . base64_encode($subject_line) . '?=', $mail_body, $headers)) {
            // Ștergem captcha DOAR după succes
            unset($_SESSION['contact_captcha_sum']);
            // Redirectare adaptată pentru structura ta de URL-uri
            header('Location: /' . $current_lang . '/contact?status=success');
            exit;
        }
    }
}

// Generăm numere noi pentru afișare în formular
  ...
  <div class="container">
    <div class="row justify-content-center">
      <div class="col-12 col-lg-8 col-xl-7 contact-narrow">
        ...
        <p class="text-center small"><a href="/<?php echo $current_lang; ?>/contact"><?php echo isset($lang['cont_send_another']) ? $lang['cont_send_another'] : 'Trimite alt mesaj'; ?></a></p>
        <?php endif; ?>
        <?php if (!empty($errors)) : ?>
        ...
        <div class="card border-0 shadow-sm contact-card mb-5">
          <div class="card-body p-4 p-md-5">
            <form class="contact-form" method="post" action="/<?php echo $current_lang; ?>/contact">
              ...
              <div class="mb-4">
                ...
                <div class="input-group"> <span class="input-group-text"><?php echo isset($lang['cont_captcha_q']) ? $lang['cont_captcha_q'] : 'Cât fac'; ?> <?php echo $captcha_n1; ?> + <?php echo $captcha_n2; ?>?</span>
        ...
        <section class="contact-alt text-center mb-4">
          <h2 class="h5"><?php echo isset($lang['cont_alt_title']) ? $lang['cont_alt_title'] : 'Alte căi'; ?></h2>
